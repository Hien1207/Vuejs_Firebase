<template>
<div>
  <div class="footer">
      <div class="footer-col" style="margin-left:5%">
            <img src="data:image/svg+xml;base64," class="logo-icons">
      </div>
      <div  class="footer-col" style="width:50%;margin-top:50px"> 
        <p style="font-size:14px">Wix.com là một nền tảng phát triển dựa trên đám mây hàng đầu với hàng triệu người dùng trên toàn thế giới. Chúng tôi giúp mọi người dễ dàng tạo sự hiện diện trên web một cách chuyên nghiệp, đẹp mắt.</p>
        <p style="font-size:14px">Quảng bá cho doanh nghiệp, giới thiệu tác phẩm nghệ thuật, mở cửa hàng trực tuyến hay đơn giản là thử nghiệm ý tưởng mới. Trình xây dựng trang web Wix có mọi thứ bạn cần để tạo một trang web miễn phí, hoàn toàn cá nhân hóa và chất lượng cao.</p>   
      </div>
  </div>
</div>
</template>

<script>
export default {

}
</script>

<style>
.footer{
  display: flex;
  font-family: Matter,sans-serif;
  width: 100%;
  height: 250px;
  border-top: 1px solid #bababb;
}
.footer-col{
    width: 30%;
    margin: 20px ;
}
.logo-icons{
    width: 50%;
    max-width: 190px;
    margin-right: 0;
    margin: 12% 40%;
}
.values{
  margin-left:20%;
}


</style>